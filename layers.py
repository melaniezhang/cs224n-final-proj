...
class Embedding(nn.Module):
    ...
    def forward(self, w, c):
        ...
        # (batch_size * seq_len, embed_size, max_word_len)
...
class CNN(nn.Module):
    def __init__(self, embed_size, hidden_size):
        ...
        self.conv2d = nn.Conv2d(embed_size, hidden_size, kernel_size=5, dilation=0, bias=True)

    def forward(self, x):
        # x shape (batch_size, embed_size, seq_len, max_word_len)
        x = self.conv2d(x)  # (batch_size, hidden_size, h_out, w_out)
        # because dilation=0, padding=0, h_out = h_in = seq_len
